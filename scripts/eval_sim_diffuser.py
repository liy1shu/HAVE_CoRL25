...
def load_obj_id_to_category(toy_dataset=None):
    ...
    if toy_dataset is None:
        ...
        for split in ["test"]: # "train-train", "train-test"
...
inference_module_class = {
    ...
    "diffuser_hispndit": FlowTrajectoryDiffuserSimulationModule_HisPNDiT
}


@hydra.main(config_path="../configs", config_name="eval_sim_uneven_object", version_base="1.3")
def main(cfg):
    ######################################################################
    # Torch settings.
    ######################################################################
    ...
    obj_cats.extend(['rod', 'knife', 'bookmark1', 'bookmark2'])
    metric_df = pd.DataFrame(
        np.zeros((len(obj_cats), 5)),
        index=obj_cats,
    ...
    for obj_id in tqdm.tqdm(obj_ids):
        ...
        if "bookmark1" in obj_id:
            obj_type = "bookmark1"
        elif "bookmark2" in obj_id:
            obj_type = "bookmark2"
        elif "knife" in obj_id:
            obj_type = "knife"
        else:
            obj_type = "rod"
        ...
        for result in trial_results:
            ...
            metric_df.loc[obj_type]["norm_dist"] += result.metric.item()
