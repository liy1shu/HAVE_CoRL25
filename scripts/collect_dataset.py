...
def generate_trajectory_uneven(dataset_path, obj_id, step_cnt=-1, gui=False, normalize_pcd=False, mode="train", delta_track_path=None, delta_ckpt_path=None):
    # determine the length of the trajectory
    step_cnt = np.random.randint(1, 10) if step_cnt == -1 else step_cnt
        
    # initialize the environment
    urdf_path = os.path.join(dataset_path, "raw", mode)
...
def generate_trajectories_uneven(dataset_path, obj_id, output_path, traj_per_joint, max_trials, normalize_pcd, mode, delta_track_path=None, delta_ckpt_path=None):
    trajectories = []
    ...
    trial_cnt = 0
    while traj_cnt < traj_per_joint and trial_cnt < max_trials:
        trajectory = generate_trajectory_uneven(dataset_path, obj_id, normalize_pcd=normalize_pcd, mode=mode, delta_track_path=delta_track_path, delta_ckpt_path=delta_ckpt_path)
        if trajectory is not None and len(trajectory["scores"]) == len(trajectory["pcds"]):
            trajectory['obj_id'] = obj_id
            trajectories.append(trajectory)
            traj_cnt += 1
...
def main_uneven(dataset_path, save_path, mode, traj_per_joint_train, max_trials_train, normalize_point_cloud=False, delta_track_path=None, delta_ckpt_path=None):
    if mode == 'train':
    ...
    for i, obj_id in tqdm(enumerate(obj_ids)):
        generate_trajectories_uneven(dataset_path, obj_id, f"{train_output_path}/{obj_id}.pkl", traj_per_joint_train, max_trials_train, normalize_point_cloud, mode, delta_track_path, delta_ckpt_path)
...
if __name__ == "__main__":
    ...
    if not args.uneven:
        ...
        main(dataset_path, output_path, train_obj_ids, test_obj_ids, movable_links, args.random_action_num, num_processes=args.num_processes, traj_per_joint_train=args.traj_per_joint_train, traj_per_joint_test=args.traj_per_joint_test, max_trials_train=args.max_trials_train, max_trials_test=args.max_trials_test)
