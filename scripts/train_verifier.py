...
def main(cfg):
    ...
    # Final model size
    model = HAVErifier(d_model=cfg.verifier.d_model, nhead=cfg.verifier.nhead, num_layers=cfg.verifier.num_layers, dim_feedforward=cfg.verifier.dim_feedforward, pn_bsz=cfg.verifier.pn_bsz, max_len=cfg.verifier.max_len)
    model = model.to(device)

    if cfg.verifier.pretrained_path is not None:
        state_dict = torch.load(cfg.verifier.pretrained_path, map_location=accelerator.device)
